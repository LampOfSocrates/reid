...
set -e
# Usage:
#   bash train_deit_s_fc512.sh [LR] [MAX_EPOCH] [DATA_FRAC]
#   bash train_deit_s_fc512.sh --resume <checkpoint_path> [extra_epochs]
...
if [ "$1" = "--resume" ]; then
    ...
    echo "  LR=$LR  DATA_FRAC=$DATA_FRAC  WARMUP=$WARMUP  EVAL_FREQ=$EVAL_FREQ"
...
SAVE_DIR="logs/$(date +%d%m_%H%M)/train_deit_s_fc512_lr${LR}_ep${MAX_EPOCH}_frac${DATA_FRAC}"
